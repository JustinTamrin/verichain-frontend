...
let acc = { name: 'Sunway University' };
...
app.post('/upload', upload.single('file'), async (req, res) => {
    ...
    const file = await bundle_file(acc, req.file.buffer, req.file.originalname);
    const cid = await uploadToIPFS(fs, Buffer.from(file));
...
app.post('/download', upload.single('file'), async (req, res) => {
    ...
    if (!cid)
        return (res.status(404).send('invalid QRcode'));
